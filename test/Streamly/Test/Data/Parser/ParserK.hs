...
import qualified Streamly.Internal.Data.Array.Type as AR
...
streamOfArray :: [Int] -> SK.StreamK m (AR.Array Int)
streamOfArray = SK.fromPure . AR.fromList

fromFold :: Property
fromFold =
    forAll (listOf $ chooseInt (min_value, max_value)) $ \ls ->
        monadicIO $ do
            s1 <- SK.parseChunks (PR.fromFold FL.sum) (streamOfArray ls)
...
fromPure =
    forAll (chooseInt (min_value, max_value)) $ \x ->
        monadicIO $ do
            s <- SK.parseChunks (PR.fromPure x) (streamOfArray [1])
            return $
                case s of
                Right r -> r == x
                Left _ -> False
...
fromEffect =
    forAll (chooseInt (min_value, max_value)) $ \x ->
        monadicIO $ do
            s <- SK.parseChunks (PR.fromEffect $ return x) (streamOfArray [1])
            return $
                case s of
                Right r -> r == x
                Left _ -> False
...
die =
    property
        $ monadicIO
            $ do
            s <- SK.parseChunks (PR.die "die test") (streamOfArray [0])
            return $
                case s of
                Right _ -> False
                Left _ -> True
...
parserFail =
    property
        $ monadicIO
            $ do
            s <- SK.parseChunks (Fail.fail err) (streamOfArray [0])
            return $
                case s of
                Right _ -> False
...
peekPass =
    forAll (chooseInt (1, max_length)) $ \list_length ->
        forAll (vectorOf list_length (chooseInt (min_value, max_value))) $ \ls ->
            monadicIO $ do
            s <- SK.parseChunks (PR.fromParser P.peek) (streamOfArray ls)
            return $
                case s of
...
peekFail =
    property $ monadicIO $ do
        s <- SK.parseChunks (PR.fromParser P.peek) (streamOfArray [])
        return $
            case s of
            Right _ -> False
            Left _ -> True
...
eofPass =
    property $ monadicIO $ do
        s <- SK.parseChunks (PR.fromParser P.eof) (streamOfArray [])
        return $
            case s of
            Right _ -> True
            Left _ -> False
...
eofFail =
    forAll (chooseInt (1, max_length)) $ \list_length ->
        forAll (vectorOf list_length (chooseInt (min_value, max_value))) $ \ls ->
            monadicIO $ do
            s <- SK.parseChunks (PR.fromParser P.eof) (streamOfArray ls)
            return $
                case s of
                Right _ -> False
                Left _ -> True
    ...
    forAll (chooseInt (mid_value, max_value)) $ \first_element ->
        forAll (listOf (chooseInt (min_value, max_value))) $ \ls_tail ->
            ...
            let
                ...
                predicate = (>= mid_value)
                parser = PR.fromParser $ P.satisfy predicate
            s <- SK.parseChunks parser (streamOfArray ls)
            return $
                case s of
...
satisfy =
    forAll (listOf (chooseInt (min_value, max_value))) $ \ls ->
        case runIdentity $ SK.parseChunks parser (streamOfArray ls) of
            Right r -> case ls of
                [] -> False
                (x : _) -> predicate x && (r == x)
            Left _ -> case ls of
...
onePass =
    forAll (chooseInt (1, max_value)) $ \int ->
        property (case runIdentity $ SK.parseChunks parser (streamOfArray [int]) of
            Right i -> i  == int
            Left _ -> False)
        where
            parser = PR.fromParser $ P.one

one :: Property
one =
    property $
        case runIdentity $ SK.parseChunks parser (streamOfArray []) of
            Left _ -> True
            Right _ -> False
    where
        parser = PR.fromParser $ P.one
        ...
        forAll (chooseInt (m, max_value)) $ \n ->
            forAll (chooseInt (m, max_value)) $ \list_length ->
                forAll (vectorOf list_length (chooseInt (min_value, max_value)))
                    $ \ls ->
                        case runIdentity $ SK.parseChunks (parser m n) (streamOfArray ls) of
                            Right parsed_list ->
                        ...
                        where
                            parser m n = PR.fromParser $ P.takeBetween m n FL.toList
...
takeBetween =
    ...
    go m n ls =
        let inputLen = Prelude.length ls
         in do
            let p = PR.fromParser $ P.takeBetween m n FL.toList
            case runIdentity $ SK.parseChunks p (streamOfArray ls) of
                Right xs ->
    ...
    forAll (chooseInt (min_value, max_value)) $ \n ->
        forAll (chooseInt (n, max_value)) $ \list_length ->
            forAll (vectorOf list_length
                ...
                case runIdentity $ SK.parseChunks (parser n) (streamOfArray ls) of
                    Right parsed_list ->
                ...
                parser n = PR.fromParser $ P.takeEQ n FL.toList
    ...
    forAll (chooseInt (min_value, max_value)) $ \n ->
        forAll (listOf (chooseInt (min_value, max_value))) $ \ls ->
            let
                ...
                parser n0 = PR.fromParser $ P.takeEQ n0 FL.toList
            in
                case runIdentity $ SK.parseChunks (parser n) (streamOfArray ls) of
                    Right parsed_list ->
                        if n <= list_length
    ...
    forAll (chooseInt (min_value, max_value)) $ \n ->
        forAll (chooseInt (n, max_value)) $ \list_length ->
            forAll (vectorOf list_length (chooseInt (min_value, max_value)))
                $ \ls ->
                    let
                        parser n0 = PR.fromParser $ P.takeGE n0 FL.toList
                    in
                        case runIdentity $ SK.parseChunks (parser n) (streamOfArray ls) of
...
nLessThanEqual0 tk ltk =
    forAll (elements [0, (-1)]) $ \n ->
        forAll (listOf arbitrary) $ \ls ->
            let
                parser n0 = PR.fromParser $ tk n0 FL.toList
            in
                case runIdentity $ SK.parseChunks (parser n) (streamOfArray ls) of
...
takeEndBy_ =
    forAll (listOf (chooseInt (min_value, max_value )))  $ \ls ->
        ...
        where
            ...
            parser = PR.fromParser $ P.takeEndBy_ predicate prsr
...
takeStartBy =
    forAll (listOf (chooseInt (min_value, max_value))) $ \ls ->
        let ls1 = 1:ls
        in
            case runIdentity $ SK.parseChunks parser (streamOfArray ls1) of
                Right parsed_list ->
            ...
            where
                predicate = odd
                parser = PR.fromParser $ P.takeStartBy predicate FL.toList
...
takeWhile =
    forAll (listOf (chooseInt (0, 1))) $ \ ls ->
        ...
        where
            predicate = (== 0)
            parser = PR.fromParser $ P.takeWhile predicate FL.toList
...
takeP =
    forAll
        ...
        $ \(takeNum, ls) ->
            case runIdentity $ SK.parseChunks (parser takeNum) (streamOfArray ls) of
                  Right parsed_list ->
            ...
            parser n = PR.fromParser $ P.takeP n (P.fromFold FL.toList)
...
takeWhile1 =
    forAll (listOf (chooseInt (0, 1))) $ \ ls ->
        ...
        where
            predicate = (== 0)
            parser = PR.fromParser $ P.takeWhile1 predicate FL.toList
...
groupBy =
    ...
    cmp = (==)
    parser = PR.fromParser $ P.groupBy cmp FL.toList
...
wordBy =
    forAll (listOf (elements [' ', 's']))
        $ \ls ->
              case runIdentity $ SK.parseChunks parser (strm ls) of
                  Right parsed -> checkListEqual parsed (words' ls)
                  Left _ -> property False

    where

    strm = SK.fromPure . AR.fromList
    predicate = (== ' ')
    parser = PR.fromParser $ P.many (P.wordBy predicate FL.toList) FL.toList
...
sequence =
    forAll (vectorOf 11 (listOf (chooseAny :: Gen Int))) $ \ ins ->
        let p xs = P.fromFold (FL.take (length xs) FL.toList)
            parser = PR.fromParser $ Prelude.sequence $ fmap p ins
         in monadicIO $ do
                outs <- run $
                        SK.parseChunks parser (streamOfArray $ concat ins)
                return $
                    case outs of
                        Right ls -> ls == ins
                        Left _ -> False
...
manyEqParseMany =
    ...
    forAll (chooseInt (1, 100)) $ \i ->
        monadicIO $ do
            ...
            r1 <- run $ SK.parseChunks (parser i) (streamOfArray lst)
    ...
    parser i = PR.fromParser $ P.many (split i) FL.toList
...
takeEndBy1 =
    forAll (listOf (chooseInt (0, 1))) $ \ls ->
        ...
        where
            ...
            parser = PR.fromParser
                        $ P.takeEndBy predicate
                        $ P.many (P.satisfy (const True)) FL.toList
...
takeEndByEsc =
    forAll (listOf (chooseInt (min_value, max_value))) $ \ls ->
        let
            ...
            parser = PR.fromParser $ P.takeEndByEsc isEsc isSep prsr0
...
takeFramedByEsc_ =
    forAll (listOf (chooseInt (min_value, max_value))) $ \ls ->
        let
            ...
            parser = PR.fromParser
                        $ P.takeFramedByEsc_ isEsc isBegin isEnd  FL.toList
...
takeStartBy_ =
     forAll (listOf (chooseInt (min_value, max_value))) $ \ls ->
        ...
        in
            ...
            where
                predicate = odd
                parser = PR.fromParser $ P.takeStartBy_ predicate FL.toList
-------------------------------------------------------------------------------
-- Main
-------------------------------------------------------------------------------
