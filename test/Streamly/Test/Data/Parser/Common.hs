...
import Streamly.Test.Common (checkListEqual, chooseInt)
...
mainCommon ptt = do
  describe (show ptt) $ do
    ...
    describe "test for sequence parser" $ do
        ...
        prop "P.take = Prelude.take" $ runParserTC ptt Streamly.Test.Data.Parser.Common.take
        prop "P.takeEQ = Prelude.take when len >= n" $ runParserTC ptt takeEQPass
        prop "P.takeEQ = Prelude.take when len >= n and fail otherwise"
            $ runParserTC ptt Streamly.Test.Data.Parser.Common.takeEQ
        prop "P.takeGE n ls = ls when len >= n" $ runParserTC ptt takeGEPass
        prop "P.takeGE n ls = ls when len >= n and fail otherwise" $ runParserTC ptt Streamly.Test.Data.Parser.Common.takeGE
