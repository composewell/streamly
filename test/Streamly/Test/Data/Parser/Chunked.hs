...
import qualified Streamly.Internal.Data.Parser.Chunked as P
...
applicative =
    forAll (listOf (chooseAny :: Gen Int)) $ \ list1 ->
        forAll (listOf (chooseAny :: Gen Int)) $ \ list2 ->
            let parser =
                    (,)
                        <$> ParserD.fromFold (FL.take (length list1) FL.toList)
...
sequence =
    forAll (vectorOf 11 (listOf (chooseAny :: Gen Int))) $ \ ins ->
        let p xs = ParserD.fromFold (FL.take (length xs) FL.toList)
         in monadicIO $ do
                outs <- run $
                        parse
...
monad =
    forAll (listOf (chooseAny :: Gen Int)) $ \ list1 ->
        forAll (listOf (chooseAny :: Gen Int)) $ \ list2 ->
            let parser = do
                    olist1 <- ParserD.fromFold (FL.take (length list1) FL.toList)
                    olist2 <- ParserD.fromFold (FL.take (length list2) FL.toList)
