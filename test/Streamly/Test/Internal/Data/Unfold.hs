...
fromStream =
    property $ \x -> do
        let strm = S.fromList x :: SerialT IO Int
        x1 <- S.toList $ S.unfold UF.fromStream strm
        x1 `shouldBe` (x :: [Int])
...
-- XXX This can be removed. The types provide correctness in some sense
effect :: Bool
...
take =
    property $ \x ->
        runIdentity $ do
            x1 <- S.toList $ S.unfold (UF.take x UF.repeatM) ()
            x2 <- S.toList $ S.take x $ S.unfold UF.repeatM ()
            return $ x1 == x2
...
-- XXX Effect testing can be removed. The types provide correctness in some sense
zipWithM :: Property
