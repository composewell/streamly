...
mapM ::
    ...
    -> Stream m Int
    -> m ()
mapM f n = composeN n $ Async.mapMUsing f return
...
o_n_heap_buffering value f =
    [ bgroup "buffered"
        [ benchIOSink value "mkAsync"
            (Stream.fold Fold.drain . Async.evalUsing f)
        ]
    ]
...
sourceFoldMapWith f value n =
    Async.concatMapUsing f Stream.fromPure $ Stream.fromList [n..n+value]
...
sourceFoldMapWithStream f value n =
    Async.concatMapUsing f Stream.fromPure
...
concatFoldableWith f value n =
    let step x =
     ...
     in Async.concatUsing f (Stream.fromList  list)
...
concatMapStreamsWith f outer inner n =
    Stream.fold Fold.drain
        $ Async.concatMapUsing f (sourceUnfoldrM inner) (sourceUnfoldrM outer n)
...
o_1_space_concatMap value f =
    value2 `seq`
        [ bgroup "concat"
            [ benchIO "concatMapUsing (n of 1)"
                  (concatMapStreamsWith f value 1)
            , benchIO "concatMapUsing (sqrt x of sqrt x)"
                  (concatMapStreamsWith f value2 value2)
            , benchIO "concatMapUsing (1 of n)"
