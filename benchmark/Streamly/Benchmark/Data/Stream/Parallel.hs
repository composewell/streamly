...
async2 count n =
    Stream.fold Fold.drain $
        sourceUnfoldrM count n `Async.parallel` sourceUnfoldrM count (n + 1)
...
{-# INLINE parallelFst #-}
parallelFst :: Int -> Int -> IO ()
parallelFst count n =
    Stream.fold Fold.drain $
        sourceUnfoldrM count n `Async.parallelFst` sourceUnfoldrM count (n + 1)

o_1_space_joining :: Int -> [Benchmark]
o_1_space_joining value =
    [ bgroup "joining"
        [ benchIOSrc1 "parallel (2 of n/2)" (async2 (value `div` 2))
        , benchIOSrc1 "concat parallel (2 of n/2)" (concatAsync2 (value `div` 2))
        , benchIOSrc1 "parallelFst (2 of n/2)" (parallelFst (value `div` 2))
        ]
    ]
