  ...
  value `seq` runMode (mode cfg) cfg benches
    [ bgroup "serially"
      ...
      , bgroup "transformation"
        ...
        , bench "reassembleBy Fav" $ nfIO $ randomRIO (5,5) >>= Ops.favBenchReassembleBy value
        , bench "reassembleBy Unfav 2" $ nfIO $ randomRIO (2,2) >>= Ops.unfavBenchReassembleBy value
        , bench "reassembleBy Unfav 5" $ nfIO $ randomRIO (5,5) >>= Ops.unfavBenchReassembleBy value
        ]
