...
wcb :: String -> IO Int
wcb file =
    File.toBytes file        -- Stream IO Word8
...
countArray arr =
      Stream.unfold Array.read arr                        -- Stream IO Word8
...
server :: Handle -> IO ()
server file =
      Stream.unfold TCP.acceptOnPort 8090           -- Stream IO Socket
    & Concur.concatMapWith (Concur.eager True) recv -- Stream IO (Array Char)
    & Stream.unfoldMany Array.read                  -- Stream IO Char
...
listDir dir =
      Dir.toEither dir         -- Stream IO (Either String String)
