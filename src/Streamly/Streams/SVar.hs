...
fromStreamVar sv = mkStream $ \st stp sng yld -> do
    ...
    -- streams.
    unStream (processEvents $ reverse list) st stp sng yld
    ...
    processEvents [] = mkStream $ \st stp sng yld -> do
        ...
        if done
        then allDone stp
        else unStream (fromStreamVar sv) st stp sng yld
    ...
    processEvents (ev : es) = mkStream $ \st stp sng yld -> do
        ...
        case ev of
            ...
            ChildStop tid e -> do
                ...
                case e of
                    Nothing -> unStream rest st stp sng yld
                    Just ex ->
                        case fromException ex of
                            Just ThreadAbort ->
                                unStream rest st stp sng yld
                            Nothing -> liftIO (cleanupSVar sv) >> throwM ex

{-# INLINE fromSVar #-}
fromSVar :: (MonadAsync m, IsStream t) => SVar Stream m a -> t m a
fromSVar sv =
    fromStream $ mkStream $ \st stp sng yld -> do
        ref <- liftIO $ newIORef ()
        ...
        unStreamShared (fromStreamVar sv{svarRef = Just ref}) st stp sng yld
    where
