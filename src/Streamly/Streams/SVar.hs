...
-------------------------------------------------------------------------------
-- Process the reverse exception channel
-------------------------------------------------------------------------------

-- | Pull exceptions (on the worker side) transmitted by the pusher/producer to
-- the pull workers
{-# NOINLINE fromStreamVarRev #-}
fromStreamVarRev :: MonadAsync m => SVar Stream m a -> Stream m a
fromStreamVarRev sv = mkStream $ \st yld sng stp -> do
    (list, _) <- liftIO $ readOutputQBasic (outputQueueRev sv)
    -- XXX Commented: list <- readOutputQ sv
    -- Reversing the output is important to guarantee that we process the
    -- outputs in the same order as they were generated by the constituent
    -- streams.
    foldStream st yld sng stp $ processEvents $ reverse list
