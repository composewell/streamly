...
import qualified Streamly.Internal.Memory.Mutable.ArrayStream as MAS
...
splitOn
    ...
    -> t m (Array Word8)
    -> t m (Array Word8)
splitOn byte s = A.unsafeFreeze `S.map` MAS.splitOn byte (A.unsafeThraw `S.map` s)
...
splitOnSuffix byte s = A.unsafeFreeze `S.map` MAS.splitOnSuffix byte (A.unsafeThraw `S.map` s)
...
compact n xs = A.unsafeFreeze `S.map` MAS.compact n (A.unsafeThraw `S.map` xs)
...
arraysOf n str = A.unsafeFreeze `S.map` MAS.arraysOf n str
...
spliceArraysLenUnsafe len buffered =
    A.unsafeFreeze `S.map` MAS.spliceArraysLenUnsafe len buffered
-}
...
spliceArraysRealloced :: forall m a. (MonadIO m, Storable a)
    => SerialT m (Array a) -> m (Array a)
spliceArraysRealloced s = A.unsafeFreeze `S.map` MAS.spliceArraysRealloced s
-}
...
toArray s = A.unsafeFreeze <$> MAS.toArray (A.unsafeThraw `S.map` s)
