...
putIndices Array{..} = Fold step initial extract
    ...
    step _ (i, x)  =
        unsafeWithArrayContents arrContents arrStart $ \ptr -> do
            ...
            then invalidIndex "putIndices" i
            else liftIO $ poke elemPtr x
            return $ FL.Partial ()
...
modifyIndices :: forall m a. (MonadIO m, Storable a)
    => D.Stream m Int -> Array a -> (a -> a) -> m ()
modifyIndices str Array{..} f = do
    D.foldlM' func (return ()) str
    where
        func _ i = do
            liftIO $ unsafeWithArrayContents arrContents arrStart $ \ptr -> do
                let elemSize = sizeOf (undefined :: a)
                ...
                then do
                    r <- peek elemPtr
                    poke elemPtr (f r)
                else invalidIndex "modifyIndices" i
...
modify Array{..} f = liftIO $ go 0
    ...
    go i = do
        unsafeWithArrayContents arrContents arrStart $ \ptr -> do
            let elemSize = sizeOf (undefined :: a)
                elemPtr = ptr `plusPtr` (elemSize * i)
            when (elemPtr `plusPtr` elemSize <= aEnd) $ do
                r <- peek elemPtr
                poke elemPtr (f r)
                go (i+1)
...
swapIndices Array{..} i1 i2 = liftIO $ do
    unsafeWithArrayContents arrContents arrStart $ \ptr -> do
        let elemSize = sizeOf (undefined :: a)
            elemPtr1 = ptr `plusPtr` (elemSize * i1)
            elemPtr2 = ptr `plusPtr` (elemSize * i2)
        when (i1 < 0 ) $ invalidIndex "swapIndices" i1
        when (i2 < 0 ) $ invalidIndex "swapIndices" i2
        r1 :: a <- peek elemPtr1
        r2 :: a <- peek elemPtr2
        poke elemPtr1 r2
        poke elemPtr2 r1
