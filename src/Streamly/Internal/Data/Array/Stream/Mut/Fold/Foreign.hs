-- |
-- Module      : Streamly.Internal.Data.Array.Stream.Mut.Fold.Foreign
...
-- Portability : GHC
--
-- Fold a stream of mutable foreign arrays. @Fold m a b@ in this module works
...
-- >>> import qualified Streamly.Internal.Data.Array.Foreign as IArray (unsafeThaw)
-- >>> import qualified Streamly.Internal.Data.Array.Stream.Mut.Foreign as ArrayStream
-- >>> import qualified Streamly.Internal.Data.Array.Stream.Mut.Fold.Foreign as ArrayFold
-- >>> import qualified Streamly.Internal.Data.Stream.IsStream as Stream (arraysOf)
-- >>> import qualified Streamly.Prelude as Stream
--
-- >>> ArrayStream.foldArr (ArrayFold.fromFold (Fold.take 7 Fold.toList)) $ Stream.map IArray.unsafeThaw $ Stream.arraysOf 5 $ Stream.fromList "hello world"
-- "hello w"
--
module Streamly.Internal.Data.Array.Stream.Mut.Fold.Foreign
    (
...
import Streamly.Internal.Data.Array.Foreign.Mut.Type (touch, Array(..))
import Streamly.Internal.Data.Parser.ParserD (Initial(..), Step(..))
import Streamly.Internal.Data.Tuple.Strict (Tuple'(..))

import qualified Streamly.Internal.Data.Array.Foreign.Mut as Array
...
fromFold (Fold.Fold fstep finitial fextract) =
    ...
    step s (Array contents start end _) = do
...
take n (Fold (ParserD.Parser step1 initial1 extract1)) =
    ...
    step (Tuple' i r) arr = do
        ...
        then do
            ...
            let !(Array contents start _ bound) = arr
                sz = sizeOf (undefined :: a)
                end = start `plusPtr` (i * sz)
                arr1 = Array contents start end bound
                remaining = negate i1
            res <- step1 r arr1
            case res of
