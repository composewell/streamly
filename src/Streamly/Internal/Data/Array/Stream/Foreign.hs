...
parseBreakK (PRD.Parser pstep initial extract) stream = do
    ...
    -- Use strictness on "cur" to keep it unboxed
    goArray !pst backBuf st (Tuple' end _) !cur
        | cur == end = do
            go pst st backBuf
    goArray !pst backBuf st fp@(Tuple' end contents) !cur = do
        x <- liftIO $ peekWith contents cur
        pRes <- pstep pst x
        let next = INDEX_NEXT(cur)
        case pRes of
...
runArrayParserDBreak ::
       forall m a b. (MonadIO m, MonadThrow m)
...
--
{-# INLINE runArrayFold #-}
runArrayFold :: (MonadIO m, MonadThrow m) =>
...
runArrayFoldBreak :: (MonadIO m, MonadThrow m) =>
