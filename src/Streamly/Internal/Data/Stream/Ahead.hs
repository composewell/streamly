...
-- | Appends two streams, some future actions may be evaluated concurrently,
-- ahead of time. The behavior is similar to 'consM' for 'AheadT' streams:
--
-- >>> stream = Stream.yieldM (delay 2) `ahead` Stream.yieldM (delay 1)
...
-- @
--
-- Any exceptions generated by concurrent evaluation are propagated to the
-- output stream as soon as they occur.
--
