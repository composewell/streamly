...
forkSVarPar ss m r = K.mkStream $ \st yld sng stp -> do
    ...
    K.foldStream st yld sng stp $ Serial.getSerialT (SVar.fromSVar sv)
...
tapAsyncK f m = K.mkStream $ \st yld sng stp -> do
    sv <- SVar.newFoldSVar st (f . Serial.getSerialT)
    K.foldStreamShared st yld sng stp
        $ Serial.getSerialT (SVar.teeToSVar sv $ Serial.Stream m)
