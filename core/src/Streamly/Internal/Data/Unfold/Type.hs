...
module Streamly.Internal.Data.Unfold.Type
    ...
    -- Constructor Naming:
    -- - "mk" prefix marks the primitive constructors. Use mkUnfoldM vs unfold
...
takeWhileM f (Unfold step1 inject1) =
    Unfold (Producer.takeWhileM f step1) inject1
...
-- | Same as 'takeEndBy' but with a monadic predicate.
--
{-# INLINE_NORMAL takeEndByM #-}
takeEndByM :: Monad m => (b -> m Bool) -> Unfold m a b -> Unfold m a b
takeEndByM f (Unfold step1 inject1) =
    Unfold (Producer.takeEndByM f step1) inject

    where

    inject a = Just <$> inject1 a

-- | End the stream generated by the 'Unfold' as soon as the predicate
-- succeeds, including the matching element in the output.
--
{-# INLINE takeEndBy #-}
takeEndBy :: Monad m => (b -> Bool) -> Unfold m a b -> Unfold m a b
