...
import GHC.Ptr (Ptr(..))
import Streamly.Internal.Data.Array.Unboxed.Mut.Type (Array, memcmp, peek, poke)
...
{-# INLINE new #-}
new :: forall a. Prim a => Int -> IO (Ring a, Ptr a)
new count = do
...
-- array.
{-# INLINE advance #-}
advance :: forall a. Prim a => Ring a -> Ptr a -> Ptr a
...
-- array.
{-# INLINE moveBy #-}
moveBy :: forall a. Prim a => Int -> Ring a -> Ptr a -> Ptr a
...
{-# INLINE unsafeInsert #-}
unsafeInsert :: Prim a => Ring a -> Ptr a -> a -> IO (Ptr a)
...
--
-- /Unimplemented/
slide :: -- forall m a. (MonadIO m, Prim a) =>
    Ring a -> a -> m (Ring a)
slide = undefined
...
getIndexUnsafe :: -- forall m a. (MonadIO m, Prim a) =>
    Ring a -> Int -> m a
...
-- /Internal/
{-# INLINE_NORMAL read #-}
read :: forall m a. (MonadIO m, Prim a) => Unfold m (Ring a, Ptr a, Int) a
...
readRev :: -- forall m a. (MonadIO m, Prim a) =>
    Unfold m (Array a) a
readRev = undefined
...
ringsOf :: -- forall m a. (MonadIO m, Prim a) =>
...
-- /Pre-release/
--
cast :: forall a b. Prim b => Ring a -> Maybe (Ring b)
cast arr =
...
unsafeFoldRingM :: forall m a b. (MonadIO m, Prim a)
...
unsafeFoldRingNM :: forall m a b. (MonadIO m, Prim a)
...
slidingWindowWith :: forall m a b. (MonadIO m, Prim a)
...
slidingWindow :: forall m a b. (MonadIO m, Prim a)
