...
-- | Given a 'Prim' type (unused first arg) and a number of bytes, return
-- how many elements of that type will completely fit in those bytes.
--
{-# INLINE bytesToElemCount #-}
bytesToElemCount :: forall a. Prim a => a -> Int -> Int
...
-- $arrayNotes
--
-- We can use a 'Prim' constraint in the Array type and the constraint can
...
-- /Pre-release/
{-# INLINE newArrayWith #-}
newArrayWith :: forall m a. (MonadIO m, Prim a)
...
newArrayAlignedUnmanaged :: forall m a. (MonadIO m, Prim a) =>
    Int -> Int -> m (Array a)
#ifdef USE_FOREIGN_PTR
...
-- alignment dictated by the 'Prim' instance of the type.
--
-- /Internal/
{-# INLINE newArrayAligned #-}
newArrayAligned :: (MonadIO m, Prim a) => Int -> Int -> m (Array a)
...
-- array is uninitialized and the allocation is aligned as per the 'Prim'
...
withNewArrayUnsafe ::
       (MonadIO m, Prim a) => Int -> (Ptr a -> m ()) -> m (Array a)
...
putIndexUnsafe :: forall m a. (MonadIO m, Prim a)
    => Int -> a -> Array a -> m ()
...
{-# INLINE putIndexPtr #-}
putIndexPtr :: forall m a. (MonadIO m, Prim a) =>
    Ptr a -> Ptr a -> Int -> a -> m ()
...
-- /Pre-release/
{-# INLINE putIndex #-}
putIndex :: (MonadIO m, Prim a) => Int -> a -> Array a -> m ()
putIndex i x arr =
    asPtrUnsafe arr
...
-- /Pre-release/
{-# INLINE putIndices #-}
putIndices :: forall m a. (MonadIO m, Prim a)
...
--
-- /Pre-release/
modifyIndexUnsafe :: forall m a b. (MonadIO m, Prim a) =>
...
modifyIndexPtr :: forall m a b. (MonadIO m, Prim a) =>
...
--
-- /Pre-release/
modifyIndex :: forall m a b. (MonadIO m, Prim a) =>
    Int -> (a -> (a, b)) -> Array a -> m b
...
modifyIndices :: forall m a . (MonadIO m, Prim a)
...
--
-- /Pre-release/
modify :: forall m a. (MonadIO m, Prim a)
    => (a -> a) -> Array a -> m ()
...
{-# INLINE swapPtrs #-}
swapPtrs :: Prim a => Ptr a -> Ptr a -> IO ()
...
unsafeSwapIndices :: forall m a. (MonadIO m, Prim a)
    => Int -> Int -> Array a -> m ()
...
--
-- /Pre-release/
swapIndices :: forall m a. (MonadIO m, Prim a)
    => Int -> Int -> Array a -> m ()
...
allocBytesToBytes :: forall a. Prim a => a -> Int -> Int
allocBytesToBytes _ n = max (arrayPayloadSize n) (SIZE_OF(a))

-- | Given a 'Prim' type (unused first arg) and real allocation size
...
allocBytesToElemCount :: Prim a => a -> Int -> Int
...
--
{-# INLINABLE realloc #-}
realloc :: forall m a. (MonadIO m, Prim a) => Int -> Array a -> m (Array a)
realloc n arr =
...
reallocWith :: forall m a. (MonadIO m , Prim a) =>
       String
    -> (Int -> Int)
    -> Int
    -> Array a
...
-- /Pre-release/
{-# INLINE resize #-}
resize :: forall m a. (MonadIO m, Prim a) =>
...
-- /Pre-release/
{-# INLINE resizeExp #-}
resizeExp :: forall m a. (MonadIO m, Prim a) =>
    Int -> Array a -> m (Array a)
...
-- /Pre-release/
{-# INLINE rightSize #-}
rightSize :: forall m a. (MonadIO m, Prim a) => Array a -> m (Array a)
...
-- /Internal/
{-# INLINE snocNewEnd #-}
snocNewEnd :: (MonadIO m, Prim a) => Ptr a -> Array a -> a -> m (Array a)
...
-- /Internal/
{-# INLINE snocUnsafe #-}
snocUnsafe :: forall m a. (MonadIO m, Prim a) =>
    Array a -> a -> m (Array a)
...
-- /Internal/
{-# INLINE snocMay #-}
snocMay :: forall m a. (MonadIO m, Prim a) =>
...
snocWithRealloc :: forall m a. (MonadIO m, Prim a) =>
       (Int -> Int)
    -> Array a
    -> a
    -> m (Array a)
...
-- /Pre-release/
{-# INLINE snocWith #-}
snocWith :: forall m a. (MonadIO m, Prim a) =>
       (Int -> Int)
    -> Array a
    -> a
    -> m (Array a)
...
-- /Pre-release/
{-# INLINE snocLinear #-}
snocLinear :: forall m a. (MonadIO m, Prim a) => Array a -> a -> m (Array a)
...
-- /Pre-release/
{-# INLINE snoc #-}
snoc :: forall m a. (MonadIO m, Prim a) => Array a -> a -> m (Array a)
snoc = snocWith f
...
getIndexUnsafe :: forall m a. (MonadIO m, Prim a) => Int -> Array a -> m a
...
{-# INLINE getIndexPtr #-}
getIndexPtr :: forall m a. (MonadIO m, Prim a) =>
    Ptr a -> Ptr a -> Int -> m a
...
--
{-# INLINE getIndex #-}
getIndex :: (MonadIO m, Prim a) => Int -> Array a -> m a
getIndex i arr =
    asPtrUnsafe arr
        $ \p -> getIndexPtr p (aEnd arr) i

{-# INLINE getIndexPtrRev #-}
getIndexPtrRev :: forall m a. (MonadIO m, Prim a) =>
    Ptr a -> Ptr a -> Int -> m a
...
--
{-# INLINE getIndexRev #-}
getIndexRev :: (MonadIO m, Prim a) => Int -> Array a -> m a
getIndexRev i arr =
    asPtrUnsafe arr
...
{-# INLINE getIndices #-}
getIndices :: (MonadIO m, Prim a) => SerialT m Int -> Unfold m (Array a) a
...
getSliceUnsafe :: forall a. Prim a
    => Int -- ^ from index
...
-- /Pre-release/
{-# INLINE reverse #-}
reverse :: forall m a. (MonadIO m, Prim a) => Array a -> m ()
...
-- /Pre-release/
{-# INLINE partitionBy #-}
partitionBy :: forall m a. (MonadIO m, Prim a)
...
arraysOf :: forall m a. (MonadIO m, Prim a)
...
arrayStreamKFromStreamD :: forall m a. (MonadIO m, Prim a) =>
...
flattenArrays :: forall m a. (MonadIO m, Prim a)
...
flattenArraysRev :: forall m a. (MonadIO m, Prim a)
    => D.Stream m (Array a) -> D.Stream m a
...
producer :: forall m a. (MonadIO m, Prim a) => Producer m (Array a) a
...
-- @since 0.7.0
{-# INLINE_NORMAL read #-}
read :: forall m a. (MonadIO m, Prim a) => Unfold m (Array a) a
...
readRev :: forall m a. (MonadIO m, Prim a) => Unfold m (Array a) a
...
-- @since 0.7.0
{-# INLINE toList #-}
toList :: forall m a. (MonadIO m, Prim a) => Array a -> m [a]
...
toStreamD :: forall m a. (MonadIO m, Prim a) => Array a -> D.Stream m a
...
{-# INLINE toStreamK #-}
toStreamK :: forall m a. (MonadIO m, Prim a) => Array a -> K.Stream m a
...
toStreamDRev :: forall m a. (MonadIO m, Prim a) => Array a -> D.Stream m a
...
{-# INLINE toStreamKRev #-}
toStreamKRev :: forall m a. (MonadIO m, Prim a) => Array a -> K.Stream m a
...
{-# INLINE_NORMAL foldl' #-}
foldl' :: (MonadIO m, Prim a) => (b -> a -> b) -> b -> Array a -> m b
foldl' f z arr = D.foldl' f z $ toStreamD arr

-- | Right fold of an array.
{-# INLINE_NORMAL foldr #-}
foldr :: (MonadIO m, Prim a) => (a -> b -> b) -> b -> Array a -> m b
...
appendNUnsafe :: forall m a. (MonadIO m, Prim a) =>
       m (Array a)
    -> Int
    -> Fold m a (Array a)
...
appendN :: forall m a. (MonadIO m, Prim a) =>
...
-- /Pre-release/
{-# INLINE appendWith #-}
appendWith :: forall m a. (MonadIO m, Prim a) =>
...
-- /Pre-release/
{-# INLINE append #-}
append :: forall m a. (MonadIO m, Prim a) =>
...
writeNWithUnsafe :: forall m a. (MonadIO m, Prim a)
...
writeNUnsafe :: forall m a. (MonadIO m, Prim a)
...
writeNWith :: forall m a. (MonadIO m, Prim a)
    => (Int -> m (Array a)) -> Int -> Fold m a (Array a)
...
writeN :: forall m a. (MonadIO m, Prim a) => Int -> Fold m a (Array a)
...
writeNAligned :: forall m a. (MonadIO m, Prim a)
...
writeNAlignedUnmanaged :: forall m a. (MonadIO m, Prim a)
    => Int -> Int -> Fold m a (Array a)
...
writeWith :: forall m a. (MonadIO m, Prim a)
    => Int -> Fold m a (Array a)
...
-- @since 0.7.0
{-# INLINE write #-}
write :: forall m a. (MonadIO m, Prim a) => Fold m a (Array a)
...
fromStreamDN :: forall m a. (MonadIO m, Prim a)
...
-- @since 0.7.0
{-# INLINABLE fromListN #-}
fromListN :: (MonadIO m, Prim a) => Int -> [a] -> m (Array a)
...
arrayStreamKLength :: (Monad m, Prim a) => K.Stream m (Array a) -> m Int
...
fromArrayStreamK :: (Prim a, MonadIO m) =>
...
-- @since 0.7.0
{-# INLINE fromList #-}
fromList :: (MonadIO m, Prim a) => [a] -> m (Array a)
...
{-# INLINE spliceCopy #-}
spliceCopy :: (MonadIO m, Prim a) => Array a -> Array a -> m (Array a)
...
-- /Pre-release/
{-# INLINE spliceWith #-}
spliceWith :: forall m a. (MonadIO m, Prim a) =>
...
-- /Pre-release/
{-# INLINE splice #-}
splice :: (MonadIO m, Prim a) => Array a -> Array a -> m (Array a)
splice = spliceWith (+)
...
-- /Pre-release/
{-# INLINE spliceExp #-}
spliceExp :: (MonadIO m, Prim a) => Array a -> Array a -> m (Array a)
...
--
-- @since 0.7.0
splitAt :: forall a. Prim a => Int -> Array a -> (Array a, Array a)
splitAt i arr@Array{..} =
...
-- /Pre-release/
--
cast :: forall a b. Prim b => Array a -> Maybe (Array b)
cast arr =
...
-- This is a Prim array, we cannot have unevaluated data in it so this is
-- just a no op.
...
-- /Pre-release/
{-# INLINE strip #-}
strip :: forall a m. (Prim a, MonadIO m) =>
...
-- /Pre-release/
{-# INLINE bubble #-}
bubble :: (MonadIO m, Prim a) => (a -> a -> Ordering) -> Array a -> m ()
bubble cmp0 arr =
    when (l > 1) $ do
