...
unsafeFreezeWithShrink :: Prim a => MA.Array a -> Array a
...
-- | @fromUnboxedPtrUnsafe foreignPtr end bound@ creates an 'Array' that starts
-- at the memory pointed by the @foreignPtr@, @end@ is the first unused
-- address.
--
-- Unsafe: Make sure that foreignPtr <= end and (end - start) is an
-- integral multiple of the element size. Only PlainPtr type UnboxedPtr is
-- supported.
--
-- /Pre-release/
--
...
fromListRevN :: {- Prim a => -} Int -> [a] -> Array a
...
fromStreamD :: forall m a. (MonadIO m, Prim a)
...
unsafeIndexIO :: forall a. Prim a => Int -> Array a -> IO a
unsafeIndexIO i arr = MA.getIndexUnsafe i (unsafeThaw arr)

-- | Return element at the specified index without checking the bounds.
{-# INLINE_NORMAL unsafeIndex #-}
unsafeIndex :: forall a. Prim a => Int -> Array a -> a
...
readRev :: forall m a. (Monad m, Prim a) => Unfold m (Array a) a
readRev = Unfold.lmap unsafeThaw $ MA.readRevWith (return . unsafeInlineIO)

{-# INLINE_NORMAL toStreamD #-}
toStreamD :: forall m a. (Monad m, Prim a) => Array a -> D.Stream m a
toStreamD arr = MA.toStreamDWith (return . unsafeInlineIO) (unsafeThaw arr)

{-# INLINE toStreamK #-}
toStreamK :: forall m a. (Monad m, Prim a) => Array a -> K.Stream m a
toStreamK arr = MA.toStreamKWith (return . unsafeInlineIO) (unsafeThaw arr)

{-# INLINE_NORMAL toStreamDRev #-}
toStreamDRev :: forall m a. (Monad m, Prim a) => Array a -> D.Stream m a
toStreamDRev arr =
    MA.toStreamDRevWith (return . unsafeInlineIO) (unsafeThaw arr)

{-# INLINE toStreamKRev #-}
toStreamKRev :: forall m a. (Monad m, Prim a) => Array a -> K.Stream m a
toStreamKRev arr =
...
toStream :: (Monad m, Prim a) => Array a -> SerialT m a
...
toStreamRev :: (Monad m, Prim a) => Array a -> SerialT m a
...
-- XXX we are assuming that Prim equality means element equality. This may
-- or may not be correct? arrcmp is 40% faster compared to stream equality.
instance (Prim a, Eq a) => Eq (Array a) where
    {-# INLINE (==) #-}
    arr1 == arr2 = unsafeInlineIO $! unsafeThaw arr1 `MA.cmp` unsafeThaw arr2

-- Since this is a Prim array we cannot have unevaluated data in it so
...
instance (Prim a, Ord a) => Ord (Array a) where
    {-# INLINE compare #-}
...
#ifdef DEVBUILD
-- Definitions using the Prim constraint from the Array type. These are to
...
XXX Why isn't Prim implicit? This does not compile unless I use the Prim
contraint.
