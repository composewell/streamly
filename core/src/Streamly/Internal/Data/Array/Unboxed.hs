...
-- 'Streamly.Internal.Data.Array.Stream.Unboxed.toArray' to concatenate N
-- arrays at once.
--
...
producer :: forall m a. (Monad m, Prim a) => Producer m (Array a) a
producer =
...
-- @since 0.8.0
{-# INLINE_NORMAL read #-}
read :: forall m a. (Monad m, Prim a) => Unfold m (Array a) a
...
unsafeRead :: forall m a. (Monad m, Prim a) => Unfold m (Array a) a
...
-- /Pre-release/
{-# INLINE getIndexRev #-}
getIndexRev :: forall a. Prim a => Int -> Array a -> Maybe a
getIndexRev i arr =
    unsafeInlineIO
        $ asPtrUnsafe arr
            $ \ptr -> do
...
-- @since 0.8.0
{-# INLINE writeLastN #-}
writeLastN :: (Prim a, MonadIO m) => Int -> Fold m a (Array a)
writeLastN n
...
genSlicesFromLen :: forall m a. (Monad m, Prim a)
    => Int -- ^ from index
    -> Int -- ^ length of the slice
...
getSlicesFromLen :: forall m a. (Monad m, Prim a)
    => Int -- ^ from index
    -> Int -- ^ length of the slice
...
-- @since 0.8.0
{-# INLINE getIndex #-}
getIndex :: forall a. Prim a => Int -> Array a -> Maybe a
getIndex i arr =
    unsafeInlineIO
        $ asPtrUnsafe arr
            $ \ptr -> do
...
-- /Pre-release/
{-# INLINE getIndices #-}
getIndices :: (Monad m, Prim a) => SerialT m Int -> Unfold m (Array a) a
...
-- @since 0.7.0
{-# INLINE runPipe #-}
runPipe :: (MonadIO m, Prim a, Prim b)
...
streamTransform :: forall m a b. (MonadIO m, Prim a, Prim b)
...
-- /Pre-release/
{-# INLINE fold #-}
fold :: forall m a b. (Monad m, Prim a) => Fold m a b -> Array a -> m b
...
-- /Pre-release/
{-# INLINE streamFold #-}
streamFold :: (Monad m, Prim a) => (SerialT m a -> m b) -> Array a -> m b
