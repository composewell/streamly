...
import Streamly.Internal.Data.Array.Stream.Fold.Unboxed (ArrayFold (..))
import qualified Streamly.Internal.Data.Array.Stream.Mut.Unboxed as AS
...
import Prelude hiding (concat, last, null, read, unlines, (!!))
            ...
            then (x : xs', xs'')
              ...
              if m == len
                then ([x], xs)
                  ...
                  let !(Array contents start end) = x
...
runArrayFoldManyD ::
  ...
  D.Stream m b
runArrayFoldManyD
  (ArrayFold (PRD.Parser pstep initial extract))
  (D.Stream step state) =
