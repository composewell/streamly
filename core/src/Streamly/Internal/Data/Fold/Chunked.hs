...
--
-- >>> f = FoldChunked.fromFold (Fold.take 7 Fold.toList)
...
-- /Pre-release/
--
newtype FoldChunked m a b = FoldChunked (ParserD.Parser (Array a) m b)
...
#endif
    Fold.Fold m a b -> FoldChunked m a b
fromFold (Fold.Fold fstep finitial fextract) =
    FoldChunked (ParserD.Parser step initial (fmap (Done 0) . fextract))
...
#endif
    ParserD.Parser a m b -> FoldChunked m a b
fromParserD (ParserD.Parser step1 initial1 extract1) =
    FoldChunked (ParserD.Parser step initial1 extract1)
...
#endif
    Parser.Parser a m b -> FoldChunked m a b
...
adaptFold :: forall m a b. (MonadIO m) =>
    Fold.Fold m (Array a) b -> FoldChunked m a b
adaptFold f = FoldChunked $ ParserD.fromFold f
...
--
-- /Pre-release/
instance Functor m => Functor (FoldChunked m a) where
    {-# INLINE fmap #-}
    fmap f (FoldChunked p) = FoldChunked $ fmap f p

-- | Map a monadic function on the output of a fold.
--
-- /Pre-release/
{-# INLINE rmapM #-}
rmapM :: Monad m => (b -> m c) -> FoldChunked m a b -> FoldChunked m a c
rmapM f (FoldChunked p) = FoldChunked $ ParserD.rmapM f p
...
--
{-# INLINE fromPure #-}
fromPure :: Monad m => b -> FoldChunked m a b
fromPure = FoldChunked . ParserD.fromPure
...
--
{-# INLINE fromEffect #-}
fromEffect :: Monad m => m b -> FoldChunked m a b
fromEffect = FoldChunked . ParserD.fromEffect
...
{-# INLINE split_ #-}
split_ :: MonadThrow m =>
    FoldChunked m x a -> FoldChunked m x b -> FoldChunked m x b
split_ (FoldChunked p1) (FoldChunked p2) =
    FoldChunked $ ParserD.noErrorUnsafeSplit_ p1 p2
...
splitWith :: MonadThrow m
    => (a -> b -> c) -> FoldChunked m x a -> FoldChunked m x b -> FoldChunked m x c
splitWith f (FoldChunked p1) (FoldChunked p2) =
    FoldChunked $ ParserD.noErrorUnsafeSplitWith f p1 p2

-- | 'Applicative' form of 'splitWith'.
-- > (<*>) = splitWith id
instance MonadThrow m => Applicative (FoldChunked m a) where
    {-# INLINE pure #-}
    pure = fromPure
...
concatMap :: MonadThrow m =>
    (b -> FoldChunked m a c) -> FoldChunked m a b -> FoldChunked m a c
concatMap func (FoldChunked p) =
    let f x = let FoldChunked y = func x in y
     in FoldChunked $ ParserD.noErrorUnsafeConcatMap f p

-- | Monad instance applies folds sequentially. Next fold can depend on the
-- output of the previous fold. See 'concatMap'.
--
-- > (>>=) = flip concatMap
instance MonadThrow m => Monad (FoldChunked m a) where
    {-# INLINE return #-}
    return = pure
...
take :: forall m a b. (Monad m, Unbox a) =>
    Int -> FoldChunked m a b -> FoldChunked m a b
take n (FoldChunked (ParserD.Parser step1 initial1 extract1)) =
    FoldChunked $ ParserD.Parser step initial extract
    ...
    iextract s = do
        ...
        return $ case r of
            ...
            _ -> error "Bug: FoldChunked take invalid state in initial"
