...
-- Portability : GHC
--
module Streamly.Internal.Data.MutArray.TH
    ( deriveSerialize
    , deriveSerializeWith
    ) where
...
import Streamly.Internal.Data.MutArray.Type (unboxSkipUnsafe, boxSkipUnsafe)
...
mkDeserializeExprOne (DataCon cname _ _ fields) =
    case fields of
        ...
        [] -> [|pure ($(conE cname), $(varE (mkName "arr0")))|]
        _ ->
            doE
                (concat
  ...
  where
    ...
    makeBind i =
        bindS
            (tupP [varP (makeA i), varP (makeArr (i + 1))])
            [|deserialize $(varE (makeArr i))|]
...
mkDeserializeExpr headTy cons =
    case cons of
        ...
        -- Unit constructor
        [(DataCon cname _ _ [])] ->
            [|pure ($(conE cname), boxSkipUnsafe arr)|]
        -- Product type
        [con] ->
            letE
                [valD (varP (mkName "arr0")) (normalB (varE _arr)) []]
                (mkDeserializeExprOne con)
        -- Sum type
        _ ->
            doE
                [ bindS
                      (tupP [varP _tag, varP (mkName "arr0")])
                      [|deserialize $(varE _arr)|]
                , noBindS
                      (caseE
...
mkSerializeExprFields fields =
    case fields of
        ...
        _ ->
            doE
                (fmap makeBind [0 .. (numFields - 1)] ++
                 [noBindS ([|pure $(varE (makeArr numFields))|])])
  where
    numFields = length fields
    -- arr1 <- serialize arr0 field0
    -- arr2 <- serialize arr1 field1
    -- ...
    makeBind i =
        bindS
            (varP (makeArr (i + 1)))
            [|serialize $(varE (makeArr i)) $(varE (mkFieldName i))|]
        ...
        [] ->
            [|error
                  ("Attempting to poke type with no constructors (" ++
                   $(lift (pprint headTy)) ++ ")")|]
        -- Unit type
        [(DataCon _ _ _ [])] -> [|pure (unboxSkipUnsafe arr)|]
        -- Product type
