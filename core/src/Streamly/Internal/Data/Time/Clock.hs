...
updateTimeVar :: Clock -> Storable.IORef MicroSecond64 -> IO ()
updateTimeVar clock timeVar = do
    t <- fromAbsTime <$> getTime clock
    Storable.modifyIORef' timeVar (const t)

{-# INLINE updateWithDelay #-}
updateWithDelay :: RealFrac a =>
    Clock -> a -> Storable.IORef MicroSecond64 -> IO ()
...
-- writing to it.
asyncClock :: Clock -> Double -> IO (ThreadId, Storable.IORef MicroSecond64)
asyncClock clock g = do
    timeVar <- Storable.newIORef 0
    updateTimeVar clock timeVar
    tid <- forkIOManaged $ forever (updateWithDelay clock g timeVar)
    return (tid, timeVar)

{-# INLINE readClock #-}
readClock :: (ThreadId, Storable.IORef MicroSecond64) -> IO MicroSecond64
readClock (_, timeVar) = Storable.readIORef timeVar
...
resetTimerExpiry :: Clock -> MicroSecond64 -> Storable.IORef MicroSecond64 -> IO ()
resetTimerExpiry clock period timeVar = do
    t <- getTime clock
    let t1 = addToAbsTime t (toRelTime period)
    Storable.modifyIORef' timeVar (const (fromAbsTime t1))

{-# INLINE processTimerTick #-}
processTimerTick :: RealFrac a =>
    Clock -> a -> Storable.IORef MicroSecond64 -> MVar () -> IO () -> IO ()
processTimerTick clock precision timeVar mvar reset = do
    threadDelay (delayTime precision)
    t <- fromAbsTime <$> getTime clock
    expiry <- Storable.readIORef timeVar
    when (t >= expiry) $ do
