    ...
    iterStep from st fstep extract final = do
        ...
        return
            ...
            $ case res of
                  ...
                  FL.Continue fs -> FIterStream st fstep fs extract final
    ...
    stepOuter _ (GroupingInit st) = do
        ...
        return
            $ case res of
                  ...
                  FL.Continue s -> Skip $ GroupingDo st s
        ...
        case res of
            Yield x s -> do
                ...
                case r of
                    ...
                    FL.Continue fs1 -> go SPEC x s fs1
                ...
                Yield x s -> do
                    ...
                    then do
                        ...
                        case r of
                            ...
                            FL.Continue fs1 -> go SPEC prev s fs1
                            FL.Done b -> return $ Yield b (GroupingInit s)
                    else do
                        r <- final acc
...
groupsRollingBy cmp (Fold fstep initial _ final) (Stream step state) =
    ...
    stepOuter _ (GroupingInit st) = do
        ...
        return
            $ case res of
                  ...
                  FL.Continue fs -> Skip $ GroupingDo st fs
    ...
    stepOuter gst (GroupingDoWith st fs previous) = do
        ...
        case res of
            ...
            FL.Continue s -> go SPEC previous st s
            FL.Done b -> return $ Yield b (GroupingInit st)
                    ...
                    then do
                        ...
                        return
                            ...
                            $ case result of
                                  ...
                                  FL.Continue fsi -> GroupingDoWith s fsi x
    ...
    stepOuter _ (WordsByInit st) = do
        ...
        return
            $ case res of
                  ...
                  FL.Continue s -> Skip $ WordsByDo st s
                ...
                then do
                    ...
                    return
                        $ case resi of
                              ...
                              FL.Continue fs1 -> Skip $ WordsByDo s fs1
                    ...
                    then do
                        ...
                        return
                            ...
                            $ case resi of
                                  ...
                                  FL.Continue fs1 -> WordsByDo s fs1
...
splitSepBySeq_ patArr (Fold fstep initial _ final) (Stream step state) =
    ...
    stepOuter _ (SplitOnSeqWordDone n fs wrd) = do
        ...
        case r of
            ...
            FL.Continue fs1 -> skip $ SplitOnSeqWordDone (n - 1) fs1 wrd
            FL.Done b -> do
    ...
    stepOuter _ (SplitOnSeqKRDone len fs rb) = do
        ...
        case r of
            ...
            FL.Continue fs1 -> skip $ SplitOnSeqKRDone (len - SIZE_OF(a)) fs1 rb1
            FL.Done b -> do
...
splitOnSuffixSeq withSep patArr (Fold fstep initial _ final) (Stream step state) =
    ...
    processYieldSingle pat x s fs = do
        ...
        then do
            ...
            case r of
                ...
                FL.Continue fs1 -> skip $ SplitOnSuffixSeqSingle fs1 s pat
                FL.Done b -> yieldReinit jump b
    ...
    stepOuter _ (SplitOnSuffixSeqWordDone n fs wrd) = do
        ...
        case r of
            ...
            FL.Continue fs1 -> skip $ SplitOnSuffixSeqWordDone (n - 1) fs1 wrd
            FL.Done b -> do
        ...
        case res of
            Yield x s -> do
                ...
                case r of
                    ...
                    FL.Continue fs1 -> go SPEC 1 wrd s fs1
                    FL.Done b -> do
        ...
        case res of
            Yield x s -> do
                ...
                case r of
                    ...
                    FL.Continue fs1 -> skip $ SOOWhole fs1 s
...
-- | Drop all non-overlapping occurrences of the given sequence from the input
-- stream.
--
-- >>> dropMatches p xs = Stream.fold Fold.toList $ Stream.dropMatches (Array.fromList p) (Stream.fromList xs)
--
-- >>> dropMatches ".." "a..b..c"
-- "abc"
--
...
{-# INLINE_NORMAL dropMatches #-}
dropMatches
    :: forall m a. (MonadIO m, Unbox a, Enum a, Eq a)
    => Array a
    -> Stream m a
    -> Stream m a
dropMatches patArr (Stream step state) =
...
data ReplaceInfixState mba rb rh ck w s a =
    ...
    | ReplaceInfixSingle s a
    | ReplaceInfixEmitSingle Int s a

    | ReplaceInfixWordInit Int !w s
    | ReplaceInfixWordLoop !w s
    | ReplaceInfixWordDone Int !w
    | ReplaceInfixEmitWord Int s

    | ReplaceInfixKRInit Int s mba
    | ReplaceInfixKRLoop s mba !rh !ck
    | ReplaceInfixKRCheck s mba !rh
    | ReplaceInfixKRDone Int rb
    | ReplaceInfixEmitKR Int s mba

-- | Replace all non-overlapping occurrences of the given sequence in the input
-- stream with a replacement sequence.
--
-- This is like 'dropMatches' except the matched pattern is replaced by the
-- supplied replacement array instead of being dropped.
--
-- >>> replaceMatches p r xs = Stream.fold Fold.toList $ Stream.replaceMatches (Array.fromList p) (Array.fromList r) (Stream.fromList xs)
--
-- >>> replaceMatches ".." "!" "a..b..c"
-- "a!b!c"
--
-- >>> replaceMatches ".." "---" "a..b..c"
-- "a---b---c"
...
-- "abc"
--
-- >>> replaceMatches "" "X" "abc"
-- "abc"
--
-- >>> replaceMatches "aa" "b" "aaaa"
-- "bb"
--
-- >>> replaceMatches "abc" "xyz" "abc"
-- "xyz"
--
-- Space: @O(n)@ where n is the length of the infix pattern.
--
{-# INLINE_NORMAL replaceMatches #-}
replaceMatches
    :: forall m a. (MonadIO m, Unbox a, Enum a, Eq a)
    => Array a
    -> Array a
    -> Stream m a
    -> Stream m a
replaceMatches patArr replArr (Stream step state) =
    Stream stepOuter (ReplaceInfixInit state)

    where

    patLen = A.length patArr
    patBytes = A.byteLength patArr
    replLen = A.length replArr
    maxIndex = patLen - 1
    maxOffset = patBytes - SIZE_OF(a)
    elemBits = SIZE_OF(a) * 8
    ...
    {-# INLINE_LATE stepOuter #-}
    stepOuter _ (ReplaceInfixInit st)
        | patLen == 0 = skip $ ReplaceInfixEmpty st
        | patLen == 1 = do
            pat <- liftIO $ A.unsafeGetIndexIO 0 patArr
            skip $ ReplaceInfixSingle st pat
        | SIZE_OF(a) * patLen <= sizeOf (Proxy :: Proxy Word) =
            skip $ ReplaceInfixWordInit 0 0 st
        | otherwise = do
            (MutArray mba _ _ _) :: MutArray a <-
                liftIO $ MutArray.emptyOf patLen
            skip $ ReplaceInfixKRInit 0 st mba

    ---------------------------
    -- Empty pattern
    ---------------------------

    stepOuter gst (ReplaceInfixEmpty st) = do
        res <- step (adaptState gst) st
        case res of
            Yield x s -> return $ Yield x (ReplaceInfixEmpty s)
            Skip s -> skip (ReplaceInfixEmpty s)
            Stop -> return Stop
    ...
    stepOuter gst (ReplaceInfixSingle st0 pat) = go SPEC st0
        ...
        go !_ !st = do
            ...
            case res of
                Yield x s ->
                    if pat == x
                    then skip $ ReplaceInfixEmitSingle 0 s pat
                    else return $ Yield x (ReplaceInfixSingle s pat)
                Skip s -> go SPEC s
                Stop -> return Stop

    stepOuter _ (ReplaceInfixEmitSingle i s pat)
        | i >= replLen = skip $ ReplaceInfixSingle s pat
        | otherwise = do
            r <- liftIO $ A.unsafeGetIndexIO i replArr
            return $ Yield r (ReplaceInfixEmitSingle (i + 1) s pat)

    ---------------------------
    -- Short Pattern - Shift Or
    ---------------------------

    stepOuter _ (ReplaceInfixWordDone 0 _) = return Stop
    stepOuter _ (ReplaceInfixWordDone n wrd) = do
        let old = elemMask .&. (wrd `shiftR` (elemBits * (n - 1)))
        return
            $ Yield
                (toEnum $ fromIntegral old)
                (ReplaceInfixWordDone (n - 1) wrd)

    stepOuter gst (ReplaceInfixWordInit idx0 wrd0 st0) =
        go SPEC idx0 wrd0 st0
        ...
        go !_ !idx !wrd !st = do
            ...
            case res of
                Yield x s -> do
                    ...
                    then
                        if wrd1 .&. wordMask == wordPat
                        then skip $ ReplaceInfixEmitWord 0 s
                        else skip $ ReplaceInfixWordLoop wrd1 s
                    else go SPEC (idx + 1) wrd1 s
                Skip s -> go SPEC idx wrd s
                Stop ->
                    if idx /= 0
                    then skip $ ReplaceInfixWordDone idx wrd
                    else return Stop

    stepOuter gst (ReplaceInfixWordLoop wrd0 st0) =
        go SPEC wrd0 st0
            ...
            case res of
                Yield x s -> do
                    let wrd1 = addToWord wrd x
                        ...
                        oldA = toEnum (fromIntegral old)
                    if wrd1 .&. wordMask == wordPat
                    then return $ Yield oldA (ReplaceInfixEmitWord 0 s)
                    else return $ Yield oldA (ReplaceInfixWordLoop wrd1 s)
                Skip s -> go SPEC wrd s
                Stop -> skip $ ReplaceInfixWordDone patLen wrd

    stepOuter _ (ReplaceInfixEmitWord i s)
        | i >= replLen = skip $ ReplaceInfixWordInit 0 0 s
        | otherwise = do
            r <- liftIO $ A.unsafeGetIndexIO i replArr
            return $ Yield r (ReplaceInfixEmitWord (i + 1) s)

    -------------------------------
    -- General Pattern - Karp Rabin
    -------------------------------

    stepOuter gst (ReplaceInfixKRInit offset st mba) = do
        res <- step (adaptState gst) st
        case res of
            Yield x s -> do
                liftIO $ pokeAt offset mba x
                ...
                then do
                    ...
                    let ringHash = A.foldl' addCksum 0 arr
                    if ringHash == patHash && A.byteEq arr patArr
                    then skip $ ReplaceInfixKRCheck s mba 0
                    else skip $ ReplaceInfixKRLoop s mba 0 ringHash
                else skip $ ReplaceInfixKRInit (offset + SIZE_OF(a)) s mba
            Skip s -> skip $ ReplaceInfixKRInit offset s mba
            Stop -> do
                let rb = RingArray
                        { ringContents = mba
                        , ringSize = offset
                ...
                skip $ ReplaceInfixKRDone offset rb

    stepOuter gst (ReplaceInfixKRLoop st0 mba rh0 cksum0) =
        go SPEC st0 rh0 cksum0
            ...
            case res of
                Yield x s -> do
                    ...
                    let cksum1 = deltaCksum cksum old x
                        rh1 = ringHead rb1
                    if cksum1 == patHash
                    then return $ Yield old (ReplaceInfixKRCheck s mba rh1)
                    else return $ Yield old (ReplaceInfixKRLoop s mba rh1 cksum1)
                Skip s -> go SPEC s rh cksum
                Stop -> skip $ ReplaceInfixKRDone patBytes rb

    stepOuter _ (ReplaceInfixKRCheck st mba rh) = do
        let rb = RingArray
                    { ringContents = mba
                    , ringSize = patBytes
                    , ringHead = rh
        ...
        res <- liftIO $ RB.eqArray rb patArr
        if res
        then skip $ ReplaceInfixEmitKR 0 st mba
        else skip $ ReplaceInfixKRLoop st mba rh patHash

    stepOuter _ (ReplaceInfixKRDone 0 _) = return Stop
    stepOuter _ (ReplaceInfixKRDone len rb) = do
        assert (len >= 0) (return ())
        old <- RB.unsafeGetHead rb
        let rb1 = RB.moveForward rb
        return $ Yield old (ReplaceInfixKRDone (len - SIZE_OF(a)) rb1)

    stepOuter _ (ReplaceInfixEmitKR i s mba)
        | i >= replLen = skip $ ReplaceInfixKRInit 0 s mba
        | otherwise = do
            r <- liftIO $ A.unsafeGetIndexIO i replArr
...
data DropSuffixState mba rb rh w s a =
    ...
    | DropSuffixSingle s a
    | DropSuffixSingleHold s a a

    | DropSuffixWordInit Int !w s
    | DropSuffixWordLoop !w s
    | DropSuffixWordDone Int !w

    | DropSuffixKRInit Int s mba
    | DropSuffixKRLoop s mba !rh
    | DropSuffixKRDone Int rb

-- | Drop the suffix pattern from the input stream if it matches the tail of
-- the stream. Otherwise, the stream is passed through unchanged.
--
-- >>> dropSuffix p xs = Stream.fold Fold.toList $ Stream.dropSuffix (Array.fromList p) (Stream.fromList xs)
--
-- >>> dropSuffix ".." "ab.."
-- "ab"
--
...
--
-- See also 'Streamly.Internal.Data.Stream.Eliminate.stripSuffix'.
{-# INLINE_NORMAL dropSuffix #-}
dropSuffix
    :: forall m a. (MonadIO m, Unbox a, Enum a, Eq a)
    => Array a
    -> Stream m a
    -> Stream m a
dropSuffix patArr (Stream step state) =
    Stream stepOuter (DropSuffixInit state)
    ...
    {-# INLINE_LATE stepOuter #-}
    stepOuter _ (DropSuffixInit st)
        | patLen == 0 = skip $ DropSuffixEmpty st
        | patLen == 1 = do
            pat <- liftIO $ A.unsafeGetIndexIO 0 patArr
            skip $ DropSuffixSingle st pat
        | SIZE_OF(a) * patLen <= sizeOf (Proxy :: Proxy Word) =
            skip $ DropSuffixWordInit 0 0 st
        | otherwise = do
            (MutArray mba _ _ _) :: MutArray a <-
                liftIO $ MutArray.emptyOf patLen
            skip $ DropSuffixKRInit 0 st mba

    stepOuter _ DropSuffixDone = return Stop

    ---------------------------
    -- Empty pattern
    ---------------------------

    stepOuter gst (DropSuffixEmpty st) = do
        res <- step (adaptState gst) st
        case res of
            Yield x s -> return $ Yield x (DropSuffixEmpty s)
    ...
    stepOuter gst (DropSuffixSingle st pat) = do
        res <- step (adaptState gst) st
        case res of
            Yield x s -> skip $ DropSuffixSingleHold s x pat
            Skip s -> skip $ DropSuffixSingle s pat
            Stop -> return Stop

    stepOuter gst (DropSuffixSingleHold st0 held0 pat) = go SPEC st0 held0
        ...
        go !_ !st !held = do
            res <- step (adaptState gst) st
            case res of
                Yield x s -> return $ Yield held (DropSuffixSingleHold s x pat)
                Skip s -> go SPEC s held
                Stop ->
                    if held == pat
                    then return Stop
                    else return $ Yield held DropSuffixDone

    ---------------------------
    -- Short Pattern - Shift Or
    ---------------------------

    stepOuter _ (DropSuffixWordDone 0 _) = return Stop
    stepOuter _ (DropSuffixWordDone n wrd) = do
        let old = elemMask .&. (wrd `shiftR` (elemBits * (n - 1)))
        return
            $ Yield
                (toEnum $ fromIntegral old)
                (DropSuffixWordDone (n - 1) wrd)

    stepOuter gst (DropSuffixWordInit idx0 wrd0 st0) =
        go SPEC idx0 wrd0 st0
        ...
        go !_ !idx !wrd !st = do
            ...
            case res of
                Yield x s -> do
                    let wrd1 = addToWord wrd x
                    if idx == maxIndex
                    then skip $ DropSuffixWordLoop wrd1 s
                    else go SPEC (idx + 1) wrd1 s
                Skip s -> go SPEC idx wrd s
                Stop ->
                    if idx /= 0
                    then skip $ DropSuffixWordDone idx wrd
                    else return Stop

    stepOuter gst (DropSuffixWordLoop wrd0 st0) =
        go SPEC wrd0 st0
                ...
                Yield x s -> do
                    let wrd1 = addToWord wrd x
                        old = (wordMask .&. wrd)
                                `shiftR` (elemBits * (patLen - 1))
                        oldA = toEnum (fromIntegral old)
                    return $ Yield oldA (DropSuffixWordLoop wrd1 s)
                Skip s -> go SPEC wrd s
                Stop ->
                    if wrd .&. wordMask == wordPat
                    then return Stop
                    else skip $ DropSuffixWordDone patLen wrd

    -------------------------------
    -- General Pattern - Ring Buffer
    -------------------------------

    stepOuter gst (DropSuffixKRInit offset st mba) = do
        res <- step (adaptState gst) st
        case res of
            Yield x s -> do
                liftIO $ pokeAt offset mba x
                if offset == maxOffset
                then skip $ DropSuffixKRLoop s mba 0
                else skip $ DropSuffixKRInit (offset + SIZE_OF(a)) s mba
            Skip s -> skip $ DropSuffixKRInit offset s mba
            Stop -> do
                let rb = RingArray
                        { ringContents = mba
                        , ringSize = offset
                ...
                skip $ DropSuffixKRDone offset rb

    stepOuter gst (DropSuffixKRLoop st0 mba rh0) =
        go SPEC st0 rh0
        ...
        go !_ !st !rh = do
            ...
            case res of
                Yield x s -> do
                    (rb1, old) <- liftIO (RB.replace rb x)
                    let rh1 = ringHead rb1
                    return $ Yield old (DropSuffixKRLoop s mba rh1)
                Skip s -> go SPEC s rh
                Stop -> do
                    matches <- liftIO $ RB.eqArray rb patArr
                    if matches
                    then return Stop
                    else skip $ DropSuffixKRDone patBytes rb

    stepOuter _ (DropSuffixKRDone 0 _) = return Stop
    stepOuter _ (DropSuffixKRDone len rb) = do
        assert (len >= 0) (return ())
        old <- RB.unsafeGetHead rb
        let rb1 = RB.moveForward rb
        return $ Yield old (DropSuffixKRDone (len - SIZE_OF(a)) rb1)
