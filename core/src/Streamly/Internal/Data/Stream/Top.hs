...
-- Portability : GHC
--
-- Top level Stream module that can use all other lower level IsStream
-- modules.
...
import Streamly.Internal.Data.Stream
    (Stream, concatM, fromStreamD, toStreamD, toStreamK)
...
joinInnerMerge :: (a -> b -> Ordering) -> Stream m a -> Stream m b -> Stream m (a, b)
...
joinLeftMap s1 s2 =
    Stream.concatM $ do
        km <- toMap s2
        return $ Stream.mapM (joinAB km) s1
...
joinOuterMap s1 s2 =
    Stream.concatM $ do
        ...
        let res1 = Stream.mapM (return . joinAB km2) $ Stream.unfold Unfold.fromList $ Map.toList km1
                    where
                    joinAB km (k, a) =
                        case k `Map.lookup` km of
        ...
        let res2 = Stream.mapMaybe (joinAB km1) $ Stream.unfold Unfold.fromList $ Map.toList km2
                    where
                    joinAB km (k, b) =
                        case k `Map.lookup` km of
                            Just _ -> Nothing
...
mergeOuterJoin :: -- Monad m =>
    (a -> b -> Ordering) -> Stream m a -> Stream m b -> Stream m (Maybe a, Maybe b)
...
differenceBy eq s1 s2 =
    concatM
        $ do
            ...
            fmap (Stream.unfold Unfold.fromList) $ foldl' (flip (List.deleteBy eq)) xs (toStreamK s2)
