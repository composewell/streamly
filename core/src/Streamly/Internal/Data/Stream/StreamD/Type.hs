...
-- | Like 'foldMany' but evalutes the fold even if the fold did not receive any
-- input, therefore, always results in a non-empty output even on an empty
...
-- stream. Unlike 'foldManyPost' it evaluates the fold only if the fold
-- receives an input, therefore, an empty input stream results in an empty
-- output stream.
--
-- Definition:
--
